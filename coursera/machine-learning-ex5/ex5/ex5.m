...
close all;
plot(lambda_vec, error_train, 'LineWidth', 2, ...
  lambda_vec, error_val, 'LineWidth', 2);
legend('Train', 'Cross Validation');
xlabel('lambda');
ylabel('Error');
...
close all;
lambda = 3;  % Best validated lambda
[error_train, error_test] = ...
    learningCurve(X_poly, y, X_poly_test, ytest, lambda);
h = figure;
set (h,'papersize',[3 2.5]);
plot(1:m, error_train, 'LineWidth', 2, 1:m, error_test, 'LineWidth', 2);
title(sprintf('Polynomial Regression Learning Curve (lambda = %f)', lambda));
xlabel('Number of training examples');
ylabel('Error');
axis([0 13 0 100]);
